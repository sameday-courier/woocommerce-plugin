...
class Services extends WP_List_Table
{
    ...
    private function getServices(int $perPage, int $pageNumber): array
    {
        ...
        foreach ($services as &$service) {
            if ($service['sameday_code'] === CarrierConstants::LOCKER_NEXT_DAY_CODE) {
                ...
                $service['name'] = TranslatorHandler::translate(
                    CarrierConstants::OOH_SERVICES_LABELS[$hostCountry]
                );
    ...
    public function column_edit($item): string
    {
        $actions = array(
            'edit' => sprintf(
                '<a href="?post_type=page&page=%s&action=%s&id=%s">Edit</a>',
                'sameday_services',
                'edit',
                (int) $item['id']
            ),
